...
function Income() {
                ...
                {list.map((income) => (
                    ...
                    >
                        ...
                        <p className="text-sm text-gray-500">
                            Added on: <span className="text-gray-700 font-medium">
    ...
    const todayIncome = incomes
        .filter((income) => dayjs(income.created_at).format("YYYY-MM-DD") === todayDate)
        .reduce((acc, income) => acc + income.amount, 0);

    const monthIncome = incomes
        .filter((income) => dayjs(income.created_at).format("YYYY-MM") === currentMonth)
        .reduce((acc, income) => acc + income.amount, 0);

    const yearIncome = incomes
        .filter((income) => dayjs(income.created_at).format("YYYY") === currentYear)
        .reduce((acc, income) => acc + income.amount, 0);
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto bg-gray-50 rounded-xl shadow-lg custom-scrollbar overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
                <h2 className="text-3xl font-bold text-gray-800">Your Income</h2>
                <button
            ...
            <div className="mb-12">
                <WaveChart labels={chartLabels} dataPoints={monthlyData} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="h-96 bg-white rounded-xl shadow flex flex-col">
                    ...
                    <div className="p-4 flex-1 custom-scrollbar overflow-y-auto">{renderIncomes(todayIncomes)}</div>
                </div>

                <div className="h-96 bg-white rounded-xl shadow flex flex-col">
                    <div className="sticky top-0 z-10 bg-white p-4 border-b">
                        <h3 className="text-lg font-semibold">This Month's Income</h3>
                    </div>
                    <div className="p-4 flex-1 custom-scrollbar overflow-y-auto">{renderIncomes(monthIncomes)}</div>
                </div>

                <div className="h-96 bg-white rounded-xl shadow flex flex-col">
                    <div className="sticky top-0 z-10 bg-white p-4 border-b">
                        <h3 className="text-lg font-semibold">This Year's Income</h3>
                    </div>
                    <div className="p-4 flex-1 custom-scrollbar overflow-y-auto">{renderIncomes(yearIncomes)}</div>
                </div>
